                ...
                <span className='position-absolute top-0 translate-middle badge rounded-pill bg-danger' style={{left : '90%' , zIndex : '1'}}> {source} </span>
                    ...
                    <img src={!imgURL?"https://www.reuters.com/pf/resources/images/reuters/reuters-default.png?d=127" : imgURL} className="card-img-top" alt="..."/>
                        <div className="card-body">
                            <h6 className={`card-title text-${this.props.mode === "dark" ? "white" : "dark"
                        }`} >{title}...</h6>
                            <p className={`card-text text-${this.props.mode === "dark" ? "white" : "dark"
                        }`} >{description}...</p> 
                            <p className='card-text'><small className='text-danger'> By {author?author :"Unknown" } on {new Date(date).toGMTString()} </small></p>
                            <a className={`bg-${this.props.mode}`} href={!newsURL?"https://www.bbc.com/news ": newsURL} rel="noopener noreferrer" target="_blank">Read More</a>
                        </div>
                </div>
            </div>
        )
